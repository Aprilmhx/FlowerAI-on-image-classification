...
from model_client3 import Net, train, val, test
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, trainloader, valloader, testloader) -> None:
        ...
        self.model_l = Net()
        self.file_path = "CLIENT3/output/model_client3.pkl" 
        self.json_path = "CLIENT3/output/client3_results.json"          
    ...
    def get_parameters(self, config: Dict[str, Scalar]):  #order 1. {}
        """
        The client 3 uses the get_parameters function to extract the current model parameters.
...
def prepare_dataset_client3(path, num_partitions, file_path, batch_size):
    with open(file_path) as file:
        json_data = json.load(file)   
        train_num_ids = json_data["client "+str(3)]["train"]
        val_num_ids = json_data["client "+str(3)]["val"]
...
def main(cfg: DictConfig):    
    trainloader, validationloader, testloader = prepare_dataset_client3(cfg.path, cfg.num_clients, cfg.json_path, cfg.batch_size)
