...
class NetS(nn.Module):
    def __init__(self, num_classes: int) -> None:
        ...
        self.model_s.fc = nn.Linear(self.input_features_s, 3)
...
class NetT(nn.Module):
    def __init__(self, num_classes: int) -> None:
        ...
        self.model_t.fc = nn.Linear(self.input_features_t, 3)
...
def get_score(hits, counts, pflag=False):
    acc = (hits[0]+hits[1]+hits[2])/(counts[0]+counts[1]+counts[2])

    if pflag:
        print("*************Metrics******************")               
        print("Healthy: {}, Benign disease: {}, Malignant disease: {}".format(hits[0]/counts[0], hits[1]/counts[1], hits[2]/counts[2]))
...
def train(net_s, net_t, trainloader, valloader, optimizer, epochs, current_round, device: str):
    ...
    T = 2
    filename = "CLIENT2/output"    
    for epoch in range(epochs):
        losses_s = []
        class_hits_s = [0.0, 0.0, 0.0] 
        class_counts_s = [0.0+1e-7, 0.0+1e-7, 0.0+1e-7] 
        conf_label_s, conf_pred_s = [], []
        
        losses_t = []
        class_hits_t = [0.0, 0.0, 0.0]  
        class_counts_t = [0.0+1e-7, 0.0+1e-7, 0.0+1e-7] 
        ...
        print('Teacher model Epoch [%d/%d] - train loss %.4f - train accuracy %.4f - train f1_micro %.4f - train f1_weighted %.4f' % (epoch+1, epochs, np.mean(losses_t), train_acc_t, train_f1_micro_t, train_f1_weighted_t))          
        ####################################Caculate metrics#####################################     
        ...
        with open(os.path.join(filename,"local_model_client2.pkl"), 'wb') as file:
...
def val(net, valloader, device: str):
    ...
    with torch.no_grad():
        losses = []
        class_hits = [0.0, 0.0, 0.0] #
        class_counts = [0.0+1e-7, 0.0+1e-7, 0.0+1e-7] #        
        correct = 0
        val_sum = 0  
