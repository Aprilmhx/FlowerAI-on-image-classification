...
from model_client1 import Net, train, val, test
from dataset_client1 import Dataset
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, trainloader, valloader, testloader) -> None:
        ...
        self.model_l = Net()
        self.file_path = "CLIENT1/output/model_client1.pkl" 
        self.json_path = "CLIENT1/output/client1_results.json"  
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    def set_parameters(self, parameters):                 #order 3.
        """Receive parameters and apply them to the local model."""
        print("client 3")
        params_dict = zip(self.model_l.state_dict().keys(), parameters)  
        ...
        self.model_l.load_state_dict(state_dict, strict=False)         
        
    def get_parameters(self, config: Dict[str, Scalar]):  #order 1. {}
        """
        The client 1 uses the get_parameters function to extract the current model parameters.
        These parameters are then sent to the central server using a communication protocol.
        """
        print("client 1")
        #return [val.cpu().numpy() for _, val in self.model.state_dict().items()]
        return [val.cpu().numpy() for _, val in list(self.model_l.state_dict().items())[:-2]]  

    def fit(self, parameters, config):                    #order 2. {'lr': 0.0001, 'weight_decay': 0.0005, 'local_epochs': 1}
        ...
        optim = torch.optim.Adam(self.model_l.parameters(), lr=lr)

        train(self.model_l, self.trainloader, self.valloader, optim, epochs, self.current_round, self.device)
...
def prepare_dataset_client1(path, num_partitions, file_path, batch_size):
    with open(file_path) as file:
        json_data = json.load(file)         
        train_num_ids = json_data["client "+str(1)]["train"]
        val_num_ids = json_data["client "+str(1)]["val"]
        test_num_ids = json_data["client "+str(1)]["test"]
...
def main(cfg: DictConfig): 
    trainloader, validationloader, testloader = prepare_dataset_client1(cfg.path, cfg.num_clients, cfg.json_path, cfg.batch_size)
