...
def prepare_dataset(path, num_partitions, file_path, batch_size):
    with open(file_path) as file:
        ...
        for num in range(num_partitions):
            train_num_ids = json_data["client "+str(num+1)]["train"]
            val_num_ids = json_data["client "+str(num+1)]["val"]
            ...
            train_dataloaders.append(
                DataLoader(train_num_dataset, batch_size=batch_size, shuffle=True, num_workers=2)
            )
            ...
            val_dataloaders.append(
                DataLoader(val_num_dataset, batch_size=batch_size, shuffle=False, num_workers=2)
            )
        ...
        test_num_dataset = Dataset(
            ...
            img_dir=os.path.join(path),
            transform=get_validation_augmentation(224,224)) 
        test_dataloaders = DataLoader(test_num_dataset, batch_size=batch_size, shuffle=False, num_workers=2)
