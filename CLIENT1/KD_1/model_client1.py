...
class NetS(nn.Module):
    def __init__(self, num_classes: int) -> None:
        ...
        self.model_s.fc = nn.Linear(self.input_features_s, num_classes)
...
class NetT(nn.Module):
    def __init__(self, num_classes: int) -> None:
        ...
        self.model_t.fc = nn.Linear(self.input_features_t, num_classes)
...
def get_score(hits, counts, pflag=False):
    acc = (hits[0]+hits[1])/(counts[0]+counts[1])

    if pflag:
        print("*************Metrics******************")               
        print("Healthy: {}, Unhealthy: {}".format(hits[0]/counts[0], hits[1]/counts[1]))
...
def train(net_s, net_t, trainloader, valloader, optimizer, epochs, current_round, device: str):       
    ...
    T = 2
    #filename = "CLIENT1/output"    
    for epoch in range(epochs):
        losses_s = []
        class_hits_s = [0.0, 0.0] 
        class_counts_s = [0.0+1e-7, 0.0+1e-7] 
        conf_label_s, conf_pred_s = [], []
        
        losses_t = []
        class_hits_t = [0.0, 0.0]  
        class_counts_t = [0.0+1e-7, 0.0+1e-7] 
        ...
        for images, labels in trainloader:
            ...
            KD_s = online_distillation_loss(outputs_s, outputs_t, labels, T, criterion)
        ...
        #with open(os.path.join(filename,"local_model_client1_"+str(current_round)+".pkl"), 'wb') as file:
...
def val(net, valloader, device: str):
    ...
    with torch.no_grad():
        losses = []
        class_hits = [0.0, 0.0] #
        class_counts = [0.0+1e-7, 0.0+1e-7] #        
        correct = 0
        val_sum = 0  
