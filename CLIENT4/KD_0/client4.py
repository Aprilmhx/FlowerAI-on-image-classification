...
from model_client4 import Net, train, val, test
from dataset_client4 import Dataset
...
class FlowerClient(fl.client.NumPyClient):
    ...
    def __init__(self, trainloader, valloader, testloader) -> None:
        ...
        self.model_l = Net()
        self.file_path = "CLIENT4/output/model_client4.pkl"   
        self.json_path = "CLIENT4/output/client4_results.json"          
    ...
    def get_parameters(self, config: Dict[str, Scalar]):  #order 1. {}
        """
        The client 4 uses the get_parameters function to extract the current model parameters.
...
def prepare_dataset_client4(path, num_partitions, file_path, batch_size):
    with open(file_path) as file:
        json_data = json.load(file)   
        train_num_ids = json_data["client "+str(4)]["train"]
        val_num_ids = json_data["client "+str(4)]["val"]
        test_num_ids = json_data["client "+str(4)]["test"]
...
def main(cfg: DictConfig):      
    trainloader, validationloader, testloader = prepare_dataset_client4(cfg.path, cfg.num_clients, cfg.json_path, cfg.batch_size)
