...
class NetS(nn.Module):
    def __init__(self, num_classes: int) -> None:
        ...
        self.model_s.fc = nn.Linear(self.input_features_s, 4)
...
class NetT(nn.Module):
    def __init__(self, num_classes: int) -> None:
        ...
        self.model_t.fc = nn.Linear(self.input_features_t, 4)
...
def train(net_s, net_t, trainloader, valloader, optimizer, epochs, current_round, device: str):       
    ...
    T = 2
    filename = "CLIENT3/output"    
    for epoch in range(epochs):
        losses_s = []
        class_hits_s = [0.0, 0.0, 0.0, 0.0] 
        class_counts_s = [0.0+1e-7, 0.0+1e-7, 0.0+1e-7, 0.0+1e-7] 
        conf_label_s, conf_pred_s = [], []
        
        losses_t = []
        class_hits_t = [0.0, 0.0, 0.0, 0.0]  
        class_counts_t = [0.0+1e-7, 0.0+1e-7, 0.0+1e-7, 0.0+1e-7] 
        ...
        with open(os.path.join(filename,"local_model_client3.pkl"), 'wb') as file:
